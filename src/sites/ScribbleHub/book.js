    ...
    async getChapterUrls() {
        const response = await fetch(this.url.origin + allChaptersPath, {
            method: 'POST', body: new URLSearchParams({
                action: 'wi_getreleases_pagination', pagenum: -1, mypostid: (await this.getBookMetaData()).postId
            })
        })
        const $ = cheerio.load(await response.text())
        return $('.toc_w')
            .map((i, node) => ({
                order: parseInt($(node).attr('order')), url: new URL($(node).find('.toc_a').attr('href'))
            }))
            .toArray()
